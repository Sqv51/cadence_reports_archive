...
OUT_ROOT="$ROOT/outputs/job7_exact_free_power"
...
run_case() {
  local bench="$1"
  local metrics="$RUN_ROOT/${bench}_exact_free_mule/metrics/core_hybrid.json"
  local vcd="$RUN_ROOT/${bench}_exact_free_mule/vcd/core_hybrid.vcd"
  local out_dir="$OUT_ROOT/$bench/oracle/reports"
  local done_report="$out_dir/power_total.rpt"

  if [[ ! -f "$metrics" ]]; then
    echo "=== Skipping $bench (missing oracle metrics) ==="
    return 0
  fi
...
PY
  then
    echo "=== Skipping $bench (zero accepted patches) ==="
    return 0
  fi
  if [[ ! -f "$vcd" ]]; then
    echo "missing accepted oracle VCD: $vcd" >&2
    exit 1
  fi
  if [[ -f "$done_report" ]]; then
    echo "=== Skipping $bench / oracle (report already present) ==="
    return 0
  fi

  echo "=== Running $bench / oracle ==="
  python3.11 "$RUNNER" \
    --design core_hybrid \
    --benchmark-name "${bench}_oracle_job7_exact_free" \
    --vcd "$vcd" \
    --vcd-scope "$SCOPE" \
    --vcd-clock-name clk \
