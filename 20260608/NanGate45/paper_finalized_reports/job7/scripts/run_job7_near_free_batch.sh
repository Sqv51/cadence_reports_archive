...
OUT_ROOT="${SCRIPT_DIR}/outputs/job7_near_free_runs"
TABLE14_DIR="${SCRIPT_DIR}/benchmarks/table14"
FILTER_LIST="${JOB7_BENCH_FILTER:-}"
TOLERANCE_PCT="${JOB7_NEAR_FREE_TOLERANCE_PCT:-0.5}"
...
run_benchmark_pair() {
  ...
  local oracle_json="${OUT_ROOT}/${bench}_near_free_relaxed_mule/metrics/core_hybrid.json"
  ...
  if [[ -f "${baseline_json}" ]]; then
    echo "=== ${bench}: near-free baseline already present, skipping ==="
  else
    echo "=== ${bench}: near-free baseline no-VCD ==="
    python3.11 "${GEN}" "${src}" \
  ...
  if [[ -f "${oracle_json}" ]]; then
    echo "=== ${bench}: near-free-relaxed-mule already present, skipping ==="
  else
    echo "=== ${bench}: near-free-relaxed-mule oracle (${TOLERANCE_PCT}% tolerance) ==="
    python3.11 "${GEN}" "${src}" \
      --benchmark-name "${bench}" \
      --config hybrid \
      --variant near-free-relaxed-mule \
      --latency-distance-threshold 2 \
      --oracle-cycle-tolerance-pct "${TOLERANCE_PCT}" \
      --output-root "${OUT_ROOT}" \
      "$@"
  fi
}
