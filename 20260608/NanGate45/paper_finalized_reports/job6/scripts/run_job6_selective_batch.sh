...
OUT_ROOT="${SCRIPT_DIR}/outputs/job6_selective_runs"
...
run_benchmark_pair() {
  ...
  local selective_json="${OUT_ROOT}/${bench}_selective_latency_hidden_mule/metrics/core_hybrid.json"
  ...
  if [[ -f "${selective_json}" ]]; then
    echo "=== ${bench}: selective-latency-hidden-mule already present, skipping ==="
  else
    echo "=== ${bench}: selective-latency-hidden-mule ==="
    python3.11 "${GEN}" "${src}" \
      --benchmark-name "${bench}" \
      --config hybrid \
      --variant selective-latency-hidden-mule \
      --output-root "${OUT_ROOT}" \
      "$@"
  fi
}
